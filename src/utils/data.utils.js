...
const ROOT_URL = `${window.location.protocol}//${window.location.hostname}:3003/api`;
...
export const getQueuedMessage = async id => {
  if (id) {
    const endPoint = `${ROOT_URL}/queues/${id}`;
    const { data } = await axios.get(endPoint);
    return data;
  } else {
    return null;
...
export const getContextData = context =>
...
export const messageTypesSubscribedTo = async entity => {
  ...
  for (let messageType of messageTypeSubscribers.messageTypes) {
    ...
    const newMessageType = Object.assign({}, messageTypeObj, {
...
export const entitiesSubscribedTo = async messageType => {
  ...
  const messageTypeSubscribers = subscribers.find(subscriber => {
    return (
      subscriber.messageType === messageType.replace(/ /g, "_").toUpperCase()
    );
  });
  if (!messageTypeSubscribers) return [];
  let entities = [];
...
export const getMessageTypeObj = async name => {
  ...
  return messagetypes.find(msgType => msgType.verboseName === newName);
};
