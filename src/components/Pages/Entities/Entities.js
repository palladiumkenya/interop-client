...
export class Entities extends Component {
  ...
  handleNewAddressLinkClick = evt =>
    this.setState({
      NewAddressInputVisible: !this.state.NewAddressInputVisible
    });
  ...
  async componentDidMount() {
    ...
    const [entity, messageTypes] = await Promise.all([
      getEntityObj(currentPage),
      messageTypesSubscribedTo(currentPage)
    ]);
    const currentAddress = await getEntityAddress(entity.id);
    if (currentAddress.hasOwnProperty("id")) {
      const date = dateFormat(
  ...
  handleNewAddressModalOpen = () =>
    this.setState({ newAddressModalOpen: true });
  ...
  messages = () => {
    ...
    const newEntityDetails = {
      ...
      content: `${
        this.state.ActiveEntityMenuItem
      } has been added successfully! However, it will remain INACTIVE until an address is added`
    };
  ...
  newAddressModal = () => {
    const submitHandler =
      !this.state.protocol || !this.state.currentAddress
        ? this.handleEmptySubmit
        : this.handleNewAddressSubmit;
    const updateAddressLink = (
      <Link to="#" onClick={this.handleNewAddressModalOpen}>
        Update Address
      </Link>
    );
    return (
  ...
  render() {
    ...
    const tableRows = this.state.entity ? (
      this.entityTable()
    ) : (
      <CircularLoader />
    );
    const formModal = this.formModal();
    const addresses = this.state.entity ? (
      ...
      <Grid columns={13}>
        ...
        <Grid.Column width={10} className="content-area">
          ...
          >
            ...
            <Link className="link-new-subscriptions" to="/Message-Subscription">
              Add subscription
            </Link>
          </Detail>
        </Grid.Column>
